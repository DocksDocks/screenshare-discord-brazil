...
15. Bind the flavour-specific local gateway relay and install Node network guards before synchronously loading Discord's bootstrap, so pre-`ready` Electron APIs remain usable. Hold the JavaScript `ready` event and Discord's `whenReady` promise until Tor and the Chromium route are authenticated, then replay the event through Electron's emitter so listener semantics remain intact. Accept only domain-form `discord.gg` targets on port 443 and return SOCKS success only after an upstream tunnel exists.
